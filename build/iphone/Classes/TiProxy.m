...
//Rare exceptions to indicate a bug in the _gsltest code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_gsltest = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_gsltest]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.gsltest.module"
